...
# Basic support for reading .obo ontology files.
...
class Term:
    def __init__(self, tid, name, synonyms=None, is_a=None, part_of=None):
        self.tid      = tid
        self.name     = name
            ...
            if (obsolete or
                ...
                if name is None and not include_nameless:
                    ...
                    if tid not in term_by_id:
                        t = Term(tid, name, synonyms, is_a, part_of)
                        all_terms.append(t)
                        term_by_id[tid] = t
                    else:
    ...
    for rootterm in rootterms:
        if not arg.separate_children:
            ...
            for n, tid, ntype in stt:
                ...
                # resetting traversed?? horrible idea; who came up
                # with this?
#                 for t in all_terms:
#                     t.traversed = False
